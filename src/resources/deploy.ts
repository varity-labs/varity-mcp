...
- **Build killed (out of memory)**: Close other applications to free RAM. Next.js builds with large dependency trees need ~2 GB of free memory. If \`varity_build\` fails with out-of-memory, close other applications and try again. Note: \`varity_deploy\` runs the same local build, so it will encounter the same memory limit — freeing RAM is the fix, not switching tools.
- **"Module not found" during build**: A required dependency is missing. Run \`npm install --legacy-peer-deps\` in your project directory. If your \`next.config.js\` is missing the \`resolve.alias\` stubs for optional UI Kit sub-modules, the easiest fix is to compare it against a fresh project created with \`varity_init\`.
